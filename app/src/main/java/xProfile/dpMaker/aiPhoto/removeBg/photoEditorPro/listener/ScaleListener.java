...
import xProfile.dpMaker.aiPhoto.removeBg.photoEditorPro.utils.ScaleGestureDetector;
import xProfile.dpMaker.aiPhoto.removeBg.photoEditorPro.utils.Vector2D;


public class ScaleListener implements OnTouchListener {
    Boolean forspiral;
    ...
    private class ScaleGestureListener extends ScaleGestureDetector.SimpleOnScaleGestureListener implements ScaleGestureDetector.OnScaleGestureListener {
        ...
        public boolean onScale(View view, ScaleGestureDetector dHANVINE_ScaleGestureDetector) {
            ...
            transformInfo.deltaScale = ScaleListener.this.isScaleEnabled ? dHANVINE_ScaleGestureDetector.getScaleFactor() : 1.0f;
            float f = 0.0f;
            transformInfo.deltaAngle = ScaleListener.this.isRotateEnabled ? Vector2D.getAngle(this.mPrevSpanVector, dHANVINE_ScaleGestureDetector.getCurrentSpanVector()) : 0.0f;
            transformInfo.deltaX = ScaleListener.this.isTranslateEnabled ? dHANVINE_ScaleGestureDetector.getFocusX() - this.mPivotX : 0.0f;
            if (ScaleListener.this.isTranslateEnabled) {
                f = dHANVINE_ScaleGestureDetector.getFocusY() - this.mPivotY;
            }
            transformInfo.deltaY = f;
            transformInfo.pivotX = this.mPivotX;
            transformInfo.pivotY = this.mPivotY;
            transformInfo.minimumScale = ScaleListener.this.minimumScale;
            transformInfo.maximumScale = ScaleListener.this.maximumScale;
            ScaleListener.this.move(view, transformInfo);
            return false;
        }
    }
    ...
    public ScaleListener(Activity activity , Boolean bool) {
